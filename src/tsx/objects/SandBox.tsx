...
export function resetSimulation() {
  ...
  for (let i = 0; i < info.count; i++) {
    universe.add_planet(Math.random()*info.width, Math.random()*info.height,Math.random()-0.5, Math.random()-0.5, info.random?(Math.random()*12)+1:12, info.random?Math.random()*1000:universe.get_mass(), new Color(planetColors[Math.floor(planetColors.length * Math.random())]).toHex())
  }
  universe.rebuild()
}
export function changePlanets(event: ChangeEvent<HTMLInputElement>) {
  ...
  if (w >= c) {
    for (let i = 0; i < (w-c); i++) {
      universe.add_planet(Math.random()*info.width, Math.random()*info.height,Math.random()-0.5, Math.random()-0.5, info.random?(Math.random()*12)+1:12, info.random?Math.random()*universe.get_mass():universe.get_mass(), new Color(planetColors[Math.floor(planetColors.length * Math.random())]).toHex())
    }
  } else {
    for (let i = 0; i < (c-w); i++) {
